...
def show_stu_one():
    ...
    # frame_one.grid()
    Label(showone_win,text="Enter exact Reference No. below to display the Particular record:-",font=("lucida grande","14")).grid(row=0,column=0,padx=50)
...
def show_stu_all():
    ...
    d_name = database_name.get()
    mydb = db.connect(host="localhost", user="root", password="1234",database=d_name)
    cursor = mydb.cursor()
...
def save_updated_data():
    ...
    mydb = db.connect(host="localhost", user="root", password="1234", database=str(d_name))
    cursor = mydb.cursor()
...
def Save_stu_Rec():
    ...
    mydb = db.connect(host="localhost", password="1234", user="root", database=dname)
    cursor = mydb.cursor()
    ...
    save = Button(frame_Dbase, text="SAVE RECORD", borderwidth=2, padx=14, bg="RosyBrown1",command=save_updated_data,state=ACTIVE)
    save.grid(pady=10, sticky=NSEW, row=13, column=1)
...
def update_win():
    ...
    Label(stu_update, text="Only enter Reference no. and rest fields program will fill on its own. ",font=("lucida grande", "10")).grid()
    Label(stu_update, text="Note:- 1.only one record is going to update at once and you can not change the reference no. ",font=("lucida grande", "10")).grid()
    Label(stu_update, text="    2.You have to entered exact reference no. otherwise it will not fill the entry fields ", font=("lucida grande", "10")).grid()
...
def add_rec():
    ...
    Label(stu_add_new, text="Reference No. need to be unique everytime Otherwise you will get an Error", font=("lucida grande", "10")).grid()
...
def show_one():
    ...
    One_label= Label(show_one_window,text="Enter BookID of the record which you want to display:-",font=("lucida grade","12"),fg="coral")
...
def done_record():
    ...
    save = Button(frame_Dbase, text="SAVE RECORD", borderwidth=2, padx=14, bg="RosyBrown1", command=save_record,state=ACTIVE)
    save.grid(pady=10, sticky=NSEW, row=8, column=1)
...
def update_record_window():
    ...
    Label(update_record_window,text="Only enter BookID and rest fields program will fill on its own ",font=("lucida grande", "10")).grid()
    Label(update_record_window, text="Only One record at a time. To update again click on 'back' and open this window again ",font=("lucida grande", "10")).grid()
...
def add_next():
    ...
    Label(add_window, text="Make sure to enter BookID unique every time.Otherwise it will give an Error!!",font=("lucida grande", "10")).grid(row=0)
...
def sub_delete():
    try:
        ...
        cursor.execute("use %s" % d_name)
        messagebox.showinfo("","Database connected succesfully")
        next_add_button = Button(database_name_entry, text="next", bg="RosyBrown1", state=ACTIVE,command=delete_rec_form)
        next_add_button.grid(row=2, column=1)
...
def submit_dname():
    ...
    global database_name_entry
    d_name = database_name.get()
    mydb = db.connect(host= "localhost", password="1234",user="root")
    cursor= mydb.cursor()
...
def submit():
    ...
    try:
        ...
        messagebox.showerror("Unknown!!!!","Unknown database or Table with name library_system already exist in this database")
        database_name_entry.withdraw()
...
def create_table():
    ...
    # mydb = db.connect(host= "loclahost",user= "root",password="1234")
    # cursor= mydb.cursor()
    # cursor.execute("")
